...
function adicionarOuEditarItem() {
    ...
    const desc = document.getElementById('item-desc').value; // Observação do item
...
/**
 * ========================================================
 * MODAL DE CADASTRO RÁPIDO & VIA CEP
 * ========================================================
 */
...
function renderizarTabelaReal(dados) {
    ...
    tbody.innerHTML = dados.map(orc => {
        ...
        const corBg = coresStatus[orc.status] || 'bg-slate-50 text-slate-500 border-slate-200';
        ...
        <tr class="hover:bg-slate-50 transition-colors">
            ...
            <td class="p-4 md:p-5 text-center"><button onclick="gerarPDFSupabase('${orcJSON}')" class="bg-white text-slate-400 hover:text-red-600 border border-slate-200 p-2 rounded-lg transition-colors shadow-sm" title="Abrir PDF"><i class="ph-bold ph-file-pdf text-lg"></i></button></td>
        </tr>`;
    }).join('');
}
...
/**
 * ========================================================
 * MOTOR DE PDF INTELIGENTE (RECONSTRUIDO)
 * ========================================================
 */
function gerarPDFSupabase(dadosCodificados) {
    ...
    // A MÁGICA DA TABELA AQUI: Criamos a Tabela inteira dentro da div
    let htmlTabela = `<table style="width: 100%; text-align: left; border-collapse: collapse; margin-bottom: 30px;">`;
    htmlTabela += `<thead style="background-color: #1e293b; color: white;">
        <tr>
            <th style="padding: 12px; font-size: 14px;">Tipo</th>
            <th style="padding: 12px; font-size: 14px;">Qtd</th>
            <th style="padding: 12px; font-size: 14px;">Descrição Serviço / Peça</th>
            <th style="padding: 12px; text-align: right; font-size: 14px;">V. Unitário</th>
            <th style="padding: 12px; text-align: right; font-size: 14px;">Subtotal</th>
        </tr>
    </thead><tbody style="border-bottom: 2px solid #e2e8f0;">`;
