// ========================================================
// AutoManager - Módulo de Orçamentos e O.S. (ARQUITETURA SÊNIOR)
// ========================================================
...
window.AppOS = {
    ...
    renderizarTabelaReal: function(dados) {
        ...
        tbody.innerHTML = dados.map(orc => {
            ...
            let btnAcao2 = `<button onclick="AppOS.abrirEdicaoOS('${orc.id}', 'dados', false)" class="w-9 h-9 flex items-center justify-center bg-white hover:bg-slate-50 border border-slate-200 rounded-lg transition shadow-sm" title="Editar"><i class="ph-bold ph-pencil-simple text-lg text-blue-500"></i></button>`;
            
            if (isFechado) {
                btnAcao1 = `<div class="w-9 h-9"></div>`; 
                btnAcao2 = `<button onclick="AppOS.abrirModalDestravar('${orc.id}')" class="w-9 h-9 flex items-center justify-center bg-slate-100 text-slate-600 hover:bg-slate-800 hover:text-white border border-slate-300 hover:border-slate-800 rounded-lg transition shadow-sm" title="Reabrir O.S (Exige Senha)"><i class="ph-bold ph-lock-key text-lg"></i></button>`;
            } else if (orc.status === 'Finalizado') {
                btnAcao1 = `<button onclick="AppOS.abrirFaturamentoDireto('${orc.id}')" class="w-9 h-9 flex items-center justify-center bg-emerald-50 text-emerald-600 hover:bg-emerald-500 hover:text-white border border-emerald-200 hover:border-emerald-500 rounded-lg transition shadow-sm" title="Faturar"><i class="ph-bold ph-money text-lg"></i></button>`;
            }
            ...
            <tr class="hover:bg-slate-50 transition-colors cursor-pointer" onclick="AppOS.abrirVisualizacaoOS('${orc.id}')">
                ...
                <td class="p-4 md:p-5 text-center" onclick="event.stopPropagation()">
                    <div class="flex items-center justify-center gap-1.5">
                        ${btnAcao1}
                        ${btnAcao2}
                        <button onclick="AppOS.abrirModalExclusao('${orc.id}', '${orc.numero_os}')" class="w-9 h-9 flex items-center justify-center bg-white text-slate-400 hover:text-red-500 border border-slate-200 rounded-lg transition shadow-sm" title="Excluir"><i class="ph-bold ph-trash text-lg"></i></button>
                        <button onclick="AppOS.gerarPDFSupabase('${orc.id}')" class="w-9 h-9 flex items-center justify-center bg-slate-800 text-white hover:bg-slate-900 border border-slate-800 rounded-lg transition shadow-sm" title="Abrir PDF"><i class="ph-bold ph-file-pdf text-lg"></i></button>
                    </div>
                </td>
            </tr>`;
        }).join('');
    ...
    renderizarPreviewFotos: function() {
        ...
        AppOS.state.imagensUploadArray.forEach((base64Str, index) => {
            ...
            let trashIcon = isTravadoGeral ? '' : `<div class="absolute inset-0 bg-black/60 hidden group-hover:flex items-center justify-center transition-all cursor-pointer" onclick="AppOS.abrirModalExcluirAnexo(${index})"><i class="ph-bold ph-trash text-white text-xl"></i></div>`;
    ...
    // 4. MÉTODOS DE RENDERIZAÇÃO FINANCEIRA E AUDITORIA
    ...
    // 5. ESTADOS, TRAVAS E FLUXO DA O.S.
    ...
    // 6. CADASTRO DE CLIENTE E VEÍCULO (RAPIDO)
    ...
    processarDestravarOS: async function() {
        ...
        try {
            ...
            AppOS.abrirEdicaoOS(AppOS.state.osParaDestravarId, 'dados', false);
...
console.log("🟢 Módulo Orçamentos Carregado, Ancorado no AppOS e Blindado 100%!");
