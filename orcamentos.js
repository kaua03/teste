...
let indexAnexoParaExcluir = null;
...
function renderizarPreviewFotos() {
    ...
    imagensUploadArray.forEach((base64Str, index) => {
        ...
        let trashIcon = isTravadoGeral ? '' : `<div class="absolute inset-0 bg-black/60 hidden group-hover:flex items-center justify-center transition-all cursor-pointer" onclick="abrirModalExcluirAnexo(${index})"><i class="ph-bold ph-trash text-white text-xl"></i></div>`;
...
function abrirModalExcluirAnexo(index) {
    indexAnexoParaExcluir = index;
...
function confirmarExclusaoAnexo() {
    if (indexAnexoParaExcluir !== null) {
        imagensUploadArray.splice(indexAnexoParaExcluir, 1);
        renderizarPreviewFotos();
        dispararAlerta("Evidência removida.", "sucesso");
    }
    ...
    for(let i=1; i<=parcelas; i++) {
        ...
        if (apenasAtualizar) {
            ...
            <div class="flex items-center gap-3 bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
                ...
                <input type="text" id="tab-parc-val-${i}" onkeyup="window.mascaraMoeda(this); checarSomaGeradorTab()" value="${valorParaInput(valorParc)}" class="w-24 border border-slate-300 p-2 rounded-lg text-xs font-black text-slate-800 outline-none focus:border-emerald-500">
...
function alternarSubTelaOrcamento(modo) {
    ...
    if (modo === 'novo') {
        ...
        document.getElementById('aba-fin').classList.add('hidden'); // Oculta botão financeiro
...
// ===================================================================================
// 4. BANCO DE DADOS (SUPABASE) E INTEGRAÇÃO FINAL
// ===================================================================================
...
async function excluirParcelaManual(id) {
    ...
    try {
        ...
        const { data: restantes } = await window.banco.from('contas_receber').select('id').like('descricao', `%O.S #${osEmEdicaoNumero}%`);
        if(!restantes || restantes.length === 0) {
             ...
             if(osParaDestravarDados) osParaDestravarDados.status = 'Finalizado';
        }
...
// ========================================================
// 5. EXPORTAÇÃO GLOBAL FORÇADA (ANTI-ERRO SPA)
// ========================================================
...
window.abrirModalExcluirAnexo = abrirModalExcluirAnexo;
window.fecharModalExcluirAnexo = fecharModalExcluirAnexo;
